...
void create_BMP(int nx, int ny)
{
	...
	int size_of_image = ((nx * 3) + staff_for_filling) * ny; // вычисление размера изображения (без инф. полей с учетом выравнивания)

	header_BMP.bfType[0] = 'B';				// заполнение информационных полей BMP файла в соответствии с документацией
	header_BMP.bfType[1] = 'M';
	header_BMP.bfSize = 54 + size_of_image;			// размер информационных структур равен 54 байтам
	header_BMP.bfReserved1 = 0;
	header_BMP.bfReserved2 = 0;
	header_BMP.bfOffBits = 54;				// количество байт лежащих перед началом массива изображения

	info_BMP.biSize = 40;					// размер структуры info_BMP в байтах
	...
	info_BMP.biBitCount = 24;				// количество байт в одном пикселе (в данном случае 3 байта на пиксель)
	info_BMP.biCompression = 0;				// без сжатия
	info_BMP.biSizeImage = size_of_image;			// размер изображения в байтах
	...
	for (int i = 0; i < ny; i++)		// цикл заполняющий массив с числовыми значениями цветов изображения случайными числами (случайными цветами)
	{
		...
		{
			if (j > (nx * 3))				// если все пиксели строки записаны и требуется дописать байты для выравнивания они дописываются
			{
				fprintf(pFile, "%c", 0);
				continue;
			}
