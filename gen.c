...
void generateCorrectMatrix(FILE *f, int isCorrectMultiplication); // Генерирует и записывает в файл правильную матрицу.
...
int main(){
    ...
    int isCorrectMatrix = rand() % 2; // Параметр генерации, отвечающий за генерирование матриц с ошибками или без.
    ...
    if (isCorrectMatrix == TRUE)
        isCorrectMultiplication = rand() % 2;

    FILE *f = fopen("input_file", "w");
    switch (isCorrectMatrix){
        case TRUE:
            for (int i = 0; i < numberOfMatrix; i++){
                generateCorrectMatrix(f, isCorrectMultiplication);
...
void generateCorrectMatrix(FILE *f, int isCorrectMultiplication){
    static int lines;
    static int cols;
    fpos_t pos;
    fgetpos(f, &pos);

    if (isCorrectMultiplication != TRUE || pos == 0)
